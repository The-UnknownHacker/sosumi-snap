...
class Filesystem:
    ...
    def download_file(url, size, path):
        ...
        with open(filename, 'wb') as f:
            with click.progressbar(remote.iter_content(1024), length=size / 1024,
                                   label="Fetching {} ...".format(filename)) as stream:
                for data in stream:
                    f.write(data)
        return filename
...
class SoftwareService:
    # macOS 12.0 (Monterey) is available in different catalogs from SoftwareScan
    catalogs = {
        "12.0": {
            "PublicRelease": "https://swscan.apple.com/content/catalogs/others/index-12.0-11.6-11.5-11.4-11.3-11.2-11.1-11.0-10.15-10.14-10.13-10.12-10.11-10.10-10.9-mountainlion-lion-snowleopard-leopard.merged-1.sucatalog"
        }
    }
    ...
    def getosinstall(self):
        ...
        for product in products:
            if products.get(product, {}).get('ExtendedMetaInfo', {}).get('InstallAssistantPackageIdentifiers',
                                                                         {}).get('OSInstall', {}) == 'com.apple.mpkg.OSInstall':
                ospackages.append(product)

        # Iterate for a specific version
        candidates = []
        for product in ospackages:
            meta_url = products.get(product, {}).get('ServerMetadataURL', {})
            if self.version in Filesystem.parse_plist(Filesystem.fetch_plist(meta_url)).get(
...
@click.option('-v', '--catalog-version', default="12.0", help="Version of catalog.")
...
def fetchmacos(output_dir="BaseSystem/", catalog_version="12.0", catalog_id="PublicRelease", product_id=""):
